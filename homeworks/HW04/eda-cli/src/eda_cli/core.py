...
def compute_quality_flags(summary: DatasetSummary, missing_df: pd.DataFrame) -> Dict[str, Any]:
    ...
    # Проверка на наличие колонок с полностью одинаковыми значениями
    has_constant = any(
    ...
    if user_id_col is not None:
        is_id_unique = user_id_col.unique == summary.n_rows - user_id_col.missing
        flags["has_suspicious_id_duplicates"] = not is_id_unique
    else:
        flags["has_suspicious_id_duplicates"] = False
        is_id_unique = True  # для score
